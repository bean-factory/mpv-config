...
--  seekbarfg_color = "E39C42", -- color of the seekbar progress and handle
    seekbarfg_color = "C7794A", -- color of the seekbar progress and handle
    ...
    seektime = "15"		-- Seconds to seek when scrolling over seekbar
}
...
local set_thumbnail_layout = {
    ...
    slimbox   = function()  set_thumbnail_above(12)
                            set_mini_above() end,
    modernx   = function()  set_thumbnail_above(30)
                            set_mini_below() end,
}
...
local osc_styles = {
    ...
    timecodes = "{\\blur0\\bord0\\1c&HFFFFFF&\\3c&H000000&\\fs28}",
    tooltip = "{\\blur1\\bord" .. user_opts.tooltipborder .. "\\1c&HFFFFFF&\\3c&H000000&\\fs28}",
    vidTitle = "{\\blur1\\bord0.5\\1c&HFFFFFF&\\3c&H0\\fs37\\q2\\fn" .. user_opts.titlefont .. "}",

    wcButtons = "{\\1c&HFFFFFF\\fs23\\fnmodernx-osc-icon}",
    wcTitle = "{\\1c&HFFFFFF\\fs29\\q2\\fn" .. user_opts.titlefont .. "}",
...
layouts = function ()
    ...
    -- Cache
    lo = add_layout("cache")
    lo.geometry = {x = osc_geo.w - 117, y = refY - 54, an = 9, w = 64, h = 20}
    lo.style = osc_styles.timecodes
...
function osc_init()
    ...
    ne.eventresponder["wheel_up_press"] =
        function (element) mp.commandv("no-osd", "seek", user_opts.seektime, "exact") end
    ...
    ne.content = function ()
        if state.rightTC_trem == 0 then
            if state.tc_ms then
                return ("-"..mp.get_property_osd("playtime-remaining/full").." / "..mp.get_property_osd("duration"))
            else
                return ("-"..mp.get_property_osd("playtime-remaining").." / "..mp.get_property_osd("duration"))
            end
...
--            ass:append(line_prefix .. line)
        ...
        ass:new_event()
        ass:pos(320, icon_y+30)
        ass:an(8)
        ass:append("{\\fs30\\1c&H0&\\1c&HFFFFFF&}Drop files or URLs to play here.")
